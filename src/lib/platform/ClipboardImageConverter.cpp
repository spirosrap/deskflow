...
constexpr size_t kBmpFileHeaderSize =
    sizeof(uint16_t) + sizeof(uint32_t) + sizeof(uint16_t) + sizeof(uint16_t) + sizeof(uint32_t);
constexpr size_t kBmpFileHeaderOffsetMagic = 0;
constexpr size_t kBmpFileHeaderOffsetPixelData = sizeof(uint16_t) + sizeof(uint32_t) + sizeof(uint16_t) + sizeof(uint16_t);

constexpr size_t kBmpInfoHeaderOffsetSize = 0;
constexpr size_t kBmpInfoHeaderOffsetWidth = kBmpInfoHeaderOffsetSize + sizeof(uint32_t);
constexpr size_t kBmpInfoHeaderOffsetHeight = kBmpInfoHeaderOffsetWidth + sizeof(int32_t);
constexpr size_t kBmpInfoHeaderOffsetPlanes = kBmpInfoHeaderOffsetHeight + sizeof(int32_t);
constexpr size_t kBmpInfoHeaderOffsetBitCount = kBmpInfoHeaderOffsetPlanes + sizeof(uint16_t);
constexpr size_t kBmpInfoHeaderOffsetCompression = kBmpInfoHeaderOffsetBitCount + sizeof(uint16_t);
constexpr size_t kBmpInfoHeaderSize = kBmpInfoHeaderOffsetCompression + sizeof(uint32_t) + sizeof(uint32_t) +
                                      sizeof(int32_t) + sizeof(int32_t) + sizeof(uint32_t) + sizeof(uint32_t);
...
std::string stripBmpFileHeader(const std::string &bmpData)
{
  ...
  if (rawBmpHeader[kBmpFileHeaderOffsetMagic] != 'B' || rawBmpHeader[kBmpFileHeaderOffsetMagic + 1] != 'M') {
    return {};
  }

  uint32_t offset = fromLEU32(rawBmpHeader + kBmpFileHeaderOffsetPixelData);
...
QImage imageFromDib(const std::string &bitmapData)
{
  ...
  const auto *rawInfoHeader = reinterpret_cast<const uint8_t *>(bitmapData.data());
  const auto infoHeaderSize = static_cast<size_t>(fromLEU32(rawInfoHeader + kBmpInfoHeaderOffsetSize));
  const auto width = fromLES32(rawInfoHeader + kBmpInfoHeaderOffsetWidth);
  const auto signedHeight = fromLES32(rawInfoHeader + kBmpInfoHeaderOffsetHeight);
  const auto planes = fromLEU16(rawInfoHeader + kBmpInfoHeaderOffsetPlanes);
  const auto bitCount = fromLEU16(rawInfoHeader + kBmpInfoHeaderOffsetBitCount);
  const auto compression = fromLEU32(rawInfoHeader + kBmpInfoHeaderOffsetCompression);
