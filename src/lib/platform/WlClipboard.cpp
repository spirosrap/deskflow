...
const int kMonitorIntervalMs = 250;
...
bool WlClipboard::has(Format format) const
{
  ...
  const auto refreshAvailability = [this]() {
    if (const auto availableTypes = getAvailableMimeTypes(); availableTypes.isEmpty()) {
      ...
      const bool bitmapReady = !bitmapMime.isEmpty() && isBitmapDataReady(availableTypes);
      const bool suppressScreenshotLabelText = hasScreenshotPlaceholderText(availableTypes) && !bitmapReady;
  ...
  if (!cacheValid) {
    refreshAvailability();
      ...
      format == Format::Bitmap && !m_cachedAvailable[static_cast<int>(Format::Bitmap)] &&
    ...
    for (int attempt = 0; attempt < kBitmapRetryChecks; ++attempt) {
      ...
      refreshAvailability();
      if (m_cachedAvailable[static_cast<int>(Format::Bitmap)] ||
          !m_cachedAvailable[static_cast<int>(Format::Text)]) {
        break;
      }
    }
...
std::string WlClipboard::get(Format format) const
{
  ...
  for (int attempt = 0; attempt < maxAttempts; ++attempt) {
    ...
    if (textLikeFormat && hasScreenshotPlaceholderText(availableTypes) && !bitmapReady) {
  ...
  while (!m_stopMonitoring) {
    ...
    try {
      ...
      const bool screenshotStateChanged =
          hasLastScreenshotState &&
          (lastBitmapReady != bitmapReady || lastHasPlaceholderText != hasPlaceholderText);
