...
void OSXClipboardTests::bitmapOnSecondItem()
{
  ...
  auto *textData =
      CFDataCreate(kCFAllocatorDefault, reinterpret_cast<const UInt8 *>(text.data()), static_cast<CFIndex>(text.size()));
  QVERIFY(textData != nullptr);
  const auto item1 = reinterpret_cast<PasteboardItemID>(1);
  QCOMPARE(
      PasteboardPutItemFlavor(pboard, item1, CFSTR("public.utf8-plain-text"), textData, kPasteboardFlavorNoFlags),
      noErr
  );
  CFRelease(textData);
